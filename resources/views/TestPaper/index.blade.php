...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Test Paper List</h5>
                </div>
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a class="mx-1 ltr:md:text-end" title="Add New Rate" href="{{ route('TestPaper.add') }}"
                                        style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add Test Paper
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$testpaper->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('TestPaper.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            <thead class="bg-slate-100 dark:bg-zink-600">
                                                <tr>
                                                    ...
                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="are_name">Test Paper Name</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="state_name">Language</th>
                                                ...
                                                @foreach ($testpaper as $testpape)
                                                    <tr>
                                                        ...
                                                        <td
                                                            ...
                                                            {{ $testpape->TestPaper_name }}</td>


                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            @if ($testpape->language == 1)
                                                                Hindi
                                                            @elseif($testpape->language == 2)
                                                                English
                                                            @elseif($testpape->language == 3)
                                                                Gujarati
                                                            @endif
                                                            ...
                                                            <div class="flex gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('TestPaper.edit', ['id' => $testpape->TestPaper_id]) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>


                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $testpape->TestPaper_id }})">
                                                                        <i class="ri-delete-bin-5-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Add Question"
                                                                        href="{{ route('question.questionlist', [$testpape->TestPaper_id, $testpape->language]) }}">
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $testpaper->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="TestPaper_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="TestPaper_ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('TestPaper.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
